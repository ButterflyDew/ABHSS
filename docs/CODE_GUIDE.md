...
`PrepareProblem` 依次构建零权分量下界、距离—根初始化，再决定是否需要真实路径并集、锚组、tour 下界和当前配置自己的 witness。正权图的 `ComputeComponentCover` 直接使用加载期最小边权，不扫描原边；两条路径都先聚合查询触及的至多 $F$ 个分量，再计算全部组子集 cover；没有 rooted 消费者时只保留全组下界和代表根，不持久保存子集表。只有开启 DirectedCut、全边严格等于 1、 $g>3$ 且 `cover_number>1` 的查询，才为 rooted-entry 构造长度 $n+1$ 的 16-bit 免费组掩码；Base、一般权图与 $g\le3$ 的共同精确基例均不支付该项。确实含零权边的图才额外扫描原边并建立并查集。对没有被零代价分量条件提前闭合的可行查询，外层始终只调用一次 `BuildDistanceRootInitialization`，并统一消费 `DistanceRootInitialization{group_distance, root, upper}`。 $g\le3$ 时全部配置统一选择 BootstrappedBounded root-star 基例并立即返回； $g>3$ 时才由冻结 profile 在 BootstrappedBounded 与 CompletePotential 之间实现同职责替换。前者在 realization 内用规范 SPT 边并集尝试启动 cutoff，再构造 bounded `GroupRow`；若非连通图的规范终端没有共同分量，bootstrap 可暂时为无穷，此时多源距离不截断，随后的共同 root-star 扫描仍会在已验证存在的公共分量中取得有限上界。后者构造完整距离势。两者都返回同一三元合同。最远组 oracle 只保存一个 byte/vertex 的全局 argmax：构造时用局部最大值做一次线性扫描，命中时 $O(1)$ 返回，未命中时严格扫描剩余 mask；完整势直接读取 dense payload，bounded 势仍走 `GroupRow` 合同。没有固定深度 top-k、查询统计或配置专属数学值。
...
`BuildOrdinaryRows` 按 mask 大小生成普通 $D$，将同根 split seed 做图闭包，并标准化 branch；递推代码只有一份，调用者只传入计划推出的最高物化层。公开入口按冻结的 `DirectedCut` 位和加载器逐边验证的 strict-unit 不变量一次分派 `BuildOrdinaryRowsImpl<staged,unit-key>`，使 flat/staged 证书链与二叉堆/整数桶的热循环不再重复读取配置；所有实例来自同一份模板递推，这项编译期特化不改变候选、证书或状态，也不是经验算法选择。size 2 的唯一规范拆分保持单遍；更高层先把全部规范拆分聚合成精确的逐顶点最小 seed，再对每个顶点运行一次与拆分无关的统一 future。Base 的 flat cache 在首次存活候选后保存完整 farthest/A1/tour 最大值；开启 `DirectedCut` 后，同一个 `CanImprove` 使用 staged realization；严格单位权特化先加入 rooted-entry，随后再进入 dual、farthest、公共 A1 与 tour，打包的 `bound_state` 以高位保存 row epoch、低 3 bit 表示 dual、farthest、公共 A1、tour 已计算到哪一步，另用 2 bit 保存 `rejected-seen` 与 `rejected-frontier`。第 4 个低位只在 `GST_ENABLE_DETAILED_PROBE_DIAGNOSTICS` 构建中记录 exact-dual 命中，论文二进制把该写入完全编译掉；epoch 仍统一左移 6 bit，使两类构建采用相同布局。strict-unit 标签可从尚未完成的 tour 阶段继续，不能把先前候选的拒绝永久化；一般加权模板首次到达 tour 就计算完整值、写缓存并结束阶段，避免较小标签反复重入部分扫描。dual 的 certified interval 下端只能作为可采纳前缀；upper endpoint 放行后，任何以后还能越过后续缓存或真实 `distance` 的标签必然更小，因而也必然放行，所以可结束 dual 判定；未转置逐组和或区间歧义触发的逐组 exact fallback 则返回可直接复用的完整值，即使当前标签被拒绝也可把 dual 阶段标为完成。这个分流只读取预声明的 DirectedCut 位，因为没有 dual 的 Base 不存在可复用的第一阶段中途拒绝；它不读取图名、 $g$、row 密度或计时，也不改变 `double` 求和。staged 路径还在同一个 32-bit `bound_state` 中保存 `rejected-seen` 与 `rejected-frontier`。仓库限制 `g <= 16`，固定锚组后 ordinary mask 少于 `2^15`；epoch 左移 6 bit 后仍小于 `2^21`，所以高位 epoch 与低 6 bit 元数据在一条查询内不会重叠或回绕。单次缓存拒绝只登记 seen；一张 row 已观察到复用后，才把虚拟拒绝值写入既有 `distance` 并加入 `rejected` 清理表。stage 0 首次物化时从 `best - bound_cache[vertex]` 得到解析 cutoff，再用 `nextafter` 修正到原 `double` 谓词确实拒绝的首个可表示值；stage 1--4 保存实际拒绝值。虚拟值不入堆、不计状态，较小标签通过完整证书链时由 crossing 分支先加入 `touched` 并清除前沿位。删除任一环节都要重新证明 row 结束复位、状态计数与候选支配安全。通过完整 future 的初始标签用 `QueueNode` 的确定性全序线性 heapify；`CanImprove` 成功时已经把完整证书写入当前 epoch 的 cache，所以 heap key 直接读取该 cache，不再经过第二个只会命中的 accessor。图松弛仍执行相同精确闭包。所有配置都读取共同 A1 future；开启 `DirectedCut` 后，统一 future 栈在 A1 之外再与对偶势取最大，而不是替换、关闭或修改 A1。`BuildReusableAnchoredSingletonLayer` 不读取配置位或 dual，三个配置使用相同的 farthest + endpoint-floor cone 与非负 fallback。DirectedCutOnly 与 Enhanced 随后都把已经生成的 A1 交给同一 `BuildForwardAnchoredRows` 内核； $H$ 只负责 A1 之后的高层后缀。补集转置扫描全部已物化 ordinary row，并为每个 H 目标生成结构上必要的单块或双块直接终端；完整 $D(Q)$ 存在时，它逐值支配同一目标的 pair。其余 split 由更大 H successor 加回一张 ordinary row 的全部精确值，最终 A/H 边界才按前向规范化要求只读 branch。非锚组二等分时，互补辅助 H row 还恢复没有 ordinary $D(h)$ 可读的平衡完成式。转置 pair 在排序枚举与互补 submask 两种等价循环中选择确定项数较少者；这一选择只改变遍历方式，不改变候选集。最高逻辑 ordinary 层已经完整物化，所以所有 A/H 边界直接读取公共 $D$ row；不再存在 separator 装箱、三块 terminal 或数据相关结构位。`complete_implicit_anchor` 仅表示完整正层域为空。

加载器确认全图边权逐条等于 1 且配置开启 DirectedCut 后，`PrepareProblem` 才构造该配置新增的单位权 rooted-entry 视图：候选诱导分量和非候选 block-cover 在组维度求覆盖数；每组一次 0-1 BFS 统计路径上不可避免的非候选顶点，并缓存每个顶点最远的两个组。`RootedEntryCoverLower` 把候选终端覆盖数与首次命中、非候选深度、block-cover 三种进入责任中的最大值相加。该下界进入 ordinary/forward/adjoint，不进入公共 A1；详细互斥计数证明见 [`METHOD.md`](METHOD.md#611-单位权图的-rooted-entry-组合下界)。

单位组距离的 `uint16_t` 视图只无损保存 $d_i(v)<U$ 的整数值，锥体外统一保存只供拒绝的 cutoff $U$；精确 DP 仍走 `ExactValueOrInf`。表示前提失败便不建视图，不做小数舍入。

`ResidualClosureScheduler` 在同一单位权图不变量下还可购买反序独立 directed-cut packing。主 residual closure 购买时先结转超过其购买价的未消费公共/A1/ordinary rent，随后继续累计真实 ordinary work；累计达到 $2m+4gn$ 的源码结构成本才构造一次；两套势始终取最大，绝不相加或逐组混合。购买后调用同一 ordinary 重滤。Base 不构造 directed-cut；DirectedCutOnly 与 Enhanced 共享这项安全新增。势转置只在加载器确认全图单位权、且逐项复核势为有限非负的 16-bit 精确整数后使用 `uint16_t` 物理布局；否则整表回退 `double`。这里没有截断、舍入或降低浮点精度。

strict-unit DirectedCut 的 ordinary 模板特化把队列容器实现为 `UnitKeyQueue`；Base 即使面对单位权图也继续使用扁平 future 与二叉堆。真实状态值仍保存为 `double`；可采纳 future 的 `ceil` 对整数真实补全代价仍安全，使接纳 key 精确落在 $[0,U)$。组合 rooted-entry 一般不具备 1-Lipschitz，故 `push` 每次都把 `current_` 回退到新 key；`top` 仍返回全局最小非空桶。正确性依赖精确整数格与显式回退，不依赖启发式一致性。若一张 row 有 $P$ 个队列项、回退后桶头前进总数为 $J$，容器工作为 $O(U+P+J)$，保守 $J\le PU$。非单位权 DirectedCut 与全部 Base 查询继续使用二叉堆。该分派不是精度压缩、桶宽超参数或按数据集选择算法。

`BuildTransposedTerminals` 只保留一份逐顶点 `ProcessVertex` 数学处理。strict-unit 模板用稀疏事件桶聚集 payload：`bucket_head[v]` 链接当前到达 $v$ 的 row，`row_next[mask]` 与 `cursor[mask]` 在消费后挂接后继。一般加权模板按 64 顶点块聚集同一批 payload，以连续块访问替代长事件链。两条路径都按递增顶点调用同一处理，并在顶点内按 `(reduced,mask)` 排序，所以候选、浮点次序和并列规则一致；边权模板只在 `SolveHighAdjoint` 入口按加载器验证的不变量分派一次，不按图名、 $g$、row 统计或耗时切换。
...
| ordinary 的其他 future | flat realization：farthest、公共 A1、tour 的完整值首次存活后缓存 | staged realization：增加主 directed-cut；strict-unit 还增加 rooted-entry/反序 packing，并允许整数 tour 部分值安全早拒绝；一般加权首次到达 tour 就完整计算并结束阶段 | rooted-entry、两套 packing 是 DirectedCut 的安全新增；所有来源只取最大。partial/full tour 是同一可采纳证书职责的物理 realization，不能把一次候选专属拒绝永久化 |
| ordinary 最小 key 容器 | 二叉堆 | 一般权图仍用二叉堆；strict-unit DirectedCut 在真实整数上界可作为 `int` 桶界时使用精确整数 key 桶 | 同一抽象全局最小 key 接口的物理 realization 替换；真实状态和 future 不变，不属于安全新增证书，也不是浮点精度压缩 |
...
| 无 Base 对应物的工作 | 无 | directed-cut 可行证书、额外 facility 收紧、延迟 residual 证书刷新、单位权反序独立 packing 与单调 D 重滤 | 安全新增；只加强下界、真实上界或删去已不能严格改善的状态；两套 packing 不共享容量预算，只能取最大 |
...
| `src/abhss/preprocess.cpp` | 正权/零权 component cover、单位权候选诱导 block-cover 与非候选 0-1 深度、组距离、 $g\le3$ 闭包、多种真实上界、tour、witness、统一 future、certificate-support subset-DP 缓存 | cutoff 不得当作精确状态；单位权 rooted-entry 的候选终端数只与互斥的进入责任相加，三种进入责任彼此取最大；`best` 只由真实可行子图收紧；support 增量失效域必须覆盖新 D mask 的全部超集；tour 上包络不能冒充新的下界 |
...
| `src/abhss/core.{h,cpp}` | A1 的 ordinary 前调度视图、ordinary $D$、逐顶点 split 聚合、flat/staged future、精确整数桶、主/反序 dual 结构购买、共同 witness rent-or-buy 与 support-DP 所有权 | A1 不读取增强位或 dual；完整 singleton pass 是 future 发布屏障；两级 A1 购买保持精确 locator/rank；ordinary 缓存不能永久化候选专属拒绝；`UnitKeyQueue` 对更小 key 必须回退，不依赖 1-Lipschitz；两套 packing 只能取最大；Base 不支付 rooted-entry、整数桶、dual stage 或 support 通知；树 DP 收紧 A1 上界时整轮重启且只发布一份标准 `Row` |
...
| `src/abhss/dual_cut.h` | `DirectedCut` 的 changed-arc 势、截断 potential cone、residual、正/反组序初始 packing、exact fallback 与 primal 边恢复 | 每个 packing 必须在自己的 residual 内容量可行；独立 packing 只能取最大；cone 只能跳过零梯度边；certified interval 不能冒充 exact；势只作下界，上界必须由原图真实边计价 |
| `src/abhss/adjoint.{h,cpp}` | strict-unit 事件桶/一般加权 64 顶点块聚集、共享逐顶点处理、ordinary 单/双块终端、辅助 $H(h)$、高层递减、互补半格与低层 $A$ 边界 | 两种聚集必须产生同一 payload 多重集，顶点内排序恢复确定全序；一般加权 prefix 首次完整缓存，strict-unit 部分 tour 可恢复；prefix 先用 farthest；strict-unit DirectedCut 还加入 rooted-entry，存活才支付 dual/tour；每个 split 必须落入完整 D、双块或 successor 全值 ordinary；空单块域不能误删双块/successor；只有最终边界读 branch；互补 H 只恢复缺失平衡式 |
...
| `abhss_configuration_exactness` | 5,000 个 $2\le g\le10$ 随机连通小图、500 个正权互异单终端实例、160 个 $g=7..16$ omitted-half 压力实例及固定辅助半层反例，逐例对照独立全子集 DP；另以 128 个单位权实例逐状态核对主/反序 packing 与 rooted-entry，以 256 个单位权实例端到端核对三种配置；并覆盖事件桶与 64 顶点块两种转置聚集、adjoint 三类 split、互补半格、A1 三种精确视图、租金表、低组闭包、逐弧 residual、weighted dual 区间等价、Base/weighted rooted 存储缺省、距离合同、共同 witness 和层计划 | 防止组合 rooted-entry 高估、独立 packing 混用容量、整数桶顺序错误、两种转置布局漏/重 payload、A1 物化漂移、缺失 terminal/successor/互补 H、potential cone 漏边、Base-only 调度复现、经验参数分派、非法配置、零权错误或 epsilon 误闭合 |
