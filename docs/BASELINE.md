...
11 个 `g=11..16` WRP 转换实例上，Safe、ABHSS-Light、ABHSS-Heavy、DPBF 与 SCIP-Jack 全部匹配已知最优值；Strict 仅 `8/11` 匹配。GPU4GST 2025 作者 CPU artifact 在 wrp4-11 返回 `179`，说明这里的反例针对本仓库对 2016 伪代码 pathmax/closed 组合的重实现，不能错误归因于 2025 artifact。机器可读证据见 [`../experiments/correctness_audit.json`](../experiments/correctness_audit.json)。
