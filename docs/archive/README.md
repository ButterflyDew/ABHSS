...
- `ADAPTIVE_A1_TOP_TWO_MATERIALIZATION_GATE_20260816.md`：共同 A1 top-two 从 lazy 二分切换到原缓存顺序物化的精确性、P1 小门、当时尚未完成的 q10 硬门及后续正式状态。
...
- `MINIMAL_FORWARD_A1_ADJOINT_GATE_20260818.md`：Enhanced 只保留共同 A1、由 H 实现其余逻辑层的正确性边界、交换绑核证据、历史 q10 checkpoint 与后续正式状态。
- `INCREMENTAL_CERTIFICATE_SUPPORT_DP_GATE_20260818.md`：固定 support 上 subset DP 的脏超集增量等价性、Orkut g15 q10 完整轨迹与直接 P1 非退化门。
- `P1_G5_G7_HISTORY_REGRESSION_PROBE_20260818.md`：当前版相对最近有效历史 P1 全量二进制的弱项定向抽样；保留 Orkut g7 q175 的时间与空间反向风险，本身不替代现已完成的正式全量 P1。
