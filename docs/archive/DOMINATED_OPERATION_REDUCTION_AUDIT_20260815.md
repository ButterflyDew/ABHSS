...
> **当前更新（2026-08-18）。** 本地 checkpoint `e9eee92` 把 Enhanced 的共同前向前缀固定为 A1，并由 H 实现逻辑层 2 到 q；其 q10 诊断时间为 9,431.057 秒。后续冻结求解器源码提交 `12d6adb` 又加入 adjoint 空域减空与 certificate-support 增量求值，并以自身二进制在 9,250.911 秒完成同一 q10，精确值 54、状态数 1,459,398,194。最终版 q10 单条门已通过，q1--q9 与 q10 组成的十条全门及完整 P1 仍待复跑，详见 [增量 support-DP 门禁](INCREMENTAL_CERTIFICATE_SUPPORT_DP_GATE_20260818.md)。
...
冻结求解器源码提交 `12d6adb` 的正式二进制 SHA-256 为 `793d4e27dfdcf52252602e4b2b8e11c3d9e06caab0a2b5f142edc2a45dc89ced`；其同源码诊断二进制已在 9,250.911 秒完成 q10，精确权值 54、状态数 1,459,398,194。因此 q10 单条子门已经通过，但完整 P1 与 q1--q10 十条全门仍未完成。最终发布前仍必须满足：
