...
用于所有最终长测的候选二进制 SHA-256 为 `a957bdcecafc486575cb7d78779dcbeda687eb0ea7283a0fc1104089dce83b86`。

### 6.3 Orkut g15 已完成直接结果

以下结果由最终候选 release 二进制直接产生。q1--q9 的答案和状态与冻结同查询结果逐条一致；q10 的旧候选从未完整结束，因此只以同一二进制的独立精确门、理论证明和全部共同 row 诊断支持其正确性，不声称不存在的历史完整轨迹对照。
...
q10 已写出完整精确结果，`query_seconds=9934.602789`，严格低于 10000 秒；外层进程还包含 30.351 秒图加载，因此判定依据是结果行而不是总墙钟。q7 也由同一二进制直接完成，权值和状态与冻结 row-local 结果一致，查询时间由 3,552.131 秒降为 3,451.310 秒。十条总查询时间为 28,862.817 秒、总状态数为 3,582,677,644，最大单查询时间和空间均来自 q10。至此 q1--q10 全部有直接结果且严格低于 10000 秒。

## 7. 接入门

只有同时满足以下条件才把候选合入当前工作树：

1. Orkut g15 q10 写出精确结果且 `query_seconds < 10000`；
2. 最终候选的 q1--q10 有足够直接证据证明全部处于同一 10000 秒门内；
3. Youtube P1 全量和 Base 静态路径均不退化；
4. 五项 CTest、源码/文档矛盾检查和 GitHub Markdown 检查全部通过。

最终候选已满足四项接入条件；失败候选只保留本归档中的方法与数字，不进入正式源码。
