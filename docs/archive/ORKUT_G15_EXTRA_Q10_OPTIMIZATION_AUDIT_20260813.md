...
最终候选 Release 二进制 SHA-256 为 `a957bdcecafc486575cb7d78779dcbeda687eb0ea7283a0fc1104089dce83b86`。Youtube P1 Enhanced 全 300 条为 2,107.963 秒、218.477 MiB RSS overhead、67,570,613 状态；与两份历史同轨迹的 300 条权值和状态逐条零差异。Base 同核前 30 条为 186.090 秒，对照为 187.848 秒，8,090,874 状态和全部权值逐条一致；随后最终候选完成 Base 全 300 条，为 1,859.266 秒、122.453 MiB、74,359,677 状态，同 CPU 冻结证据为 1,876.588 秒、128.586 MiB、相同状态，300 条权值和状态零不一致。

Orkut g15 q10 最终写出完整结果：9,934.603 秒、最优值 54、19,776.727 MiB RSS overhead、1,654,690,262 状态，严格通过 10,000 秒门。q1--q10 随后全部由同一二进制直接完成；最后补齐的 q7 为 3,451.310 秒、最优值 38、4,815.082 MiB、364,962,795 状态，与冻结同查询的权值和状态一致。十条总查询时间为 28,862.817 秒、总状态数为 3,582,677,644，最慢的 q10 仍严格低于门限。因此最终结论是“Orkut g15 十条均在 10,000 秒内完成”，不是用固定窗口进度替代完整结果。
