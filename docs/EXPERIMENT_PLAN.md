...
旧十条总查询时间为 28,862.817 秒、总状态数为 3,582,677,644；q10 为 9,934.603 秒，只剩约 65.4 秒余量。当前修复版加入辅助 $H(h)$，并只在该层保留必要 terminal；它已通过独立全子集 DP、12 点固定反例和九条已知生产差异，但 q1--q10 尚须全部重新运行。旧状态数不设为相等门，当前版按自己的精确状态域如实报告。

`mean_f` 仍不是难度的单调解释变量：旧 q10 属于最低 size stratum，却远慢于组更大的 q8；因此不能按平均组大小删除 q10 或设置经验超参数。当前复跑仍对每条查询使用固定 10,000 秒，并以冻结机器、当前 commit、当前二进制哈希和新结果行为准；若 q10 timeout，应如实计入完成率/PAR-2，并继续寻找输入无关且精确的严格支配优化，不能延长该条专属 TL。事故与修复证据见 [辅助半层 Adjoint 审计](archive/AUXILIARY_HALF_ADJOINT_CORRECTNESS_AUDIT_20260815.md)。
