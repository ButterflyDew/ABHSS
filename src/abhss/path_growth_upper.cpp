...
namespace abhss {
namespace {
// 枚举固定数量的早期组并用真实 tight paths 生长，返回最紧可行边并集。
PathGrowthUpper BuildSeededPathGrowthUpper(const Graph& graph, const Query& query, const GroupTable& distance, double cutoff, bool force_fourth) {
    ...
    auto Recover = [&](int root, int group) {
        if (distance[group].ExactValueOrInf(root) >= fp::kInf) return false;
        ++current_epoch;
        path.assign(1, root);
        path_edges.clear();
        ...
        while (!path.empty() && !(membership[path.back()] & group_bit)) {
            ...
            while (next_edge[vertex] < static_cast<int>(graph.adj[vertex].size())) {
                ...
                if (epoch[edge.to] == current_epoch) continue;
                const double next_distance = distance[group].ExactValueOrInf(edge.to);
                if (next_distance >= fp::kInf || !fp::Eq(edge.w + next_distance, current_distance)) continue;
    ...
    auto GrowAndRecord = [&](bool promising, int covered) {
        while (promising && covered != full_mask) {
            ...
            if (next_group < 0 || !CanImproveAfterConnection(next_distance) || !Recover(next_root, next_group)) break;
    ...
    for (int first = 0; first < g; ++first)
        for (int second = 0; second < g; ++second) {
            ...
            if (!start || !CanImproveAfterConnection(start_distance)) continue;
            AddVertex(start);
            ...
            for (int forced = 0; forced < g; ++forced) {
                if (forced == first || forced == second) continue;
                if (!seed_loaded) {
                    AddVertex(start);
                ...
                if (!force_fourth || (promising && covered == full_mask))
                    ...
                    for (int fourth = 0; fourth < g; ++fourth) {
                        if (third_covered & (1 << fourth)) continue;
                        if (!third_loaded) {
                            AddVertex(start);
