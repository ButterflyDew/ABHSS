...
// 枚举一张高层 H row 与普通分块 D 的同根合法和。
// singleton 分块直接读取 dense 组距离；多组分块只允许使用 ordinary row中标记为不可继续同根拆分的 branch，保持与前向完成式相同的规范分解。
template <class Use>
void ForEachBackwardBranchSum(const Problem& p, int block, const Row& backward, Use&& use) {
...
// 枚举 H successor 与新增 ordinary 块的全部精确同根和。
// 补集转置后，普通规范拆分的 branch 可能位于 successor 所代表的一侧；因此递推侧不能再次要求新增块为 branch。额外候选仍是两棵互斥 rooted子树的真实并，只会增加等价可行推导，不会得到虚假的较低值。
template <class Use>
...
// 按顶点转置 ordinary 状态，为每个物化 H 层补齐直接 ordinary split。
// 对目标 H(S)=D(full\S)，若一个规范 split 的较大一侧超过 ordinary 边界，它由已完成的 successor H 加回较小 ordinary 块；若两侧都不超过边界，则必须在这里直接转置。辅助半层仍负责恢复被省略的首张 D，较低层的直接 terminal 负责补齐 successor 递推无法表示的平衡 split。所有候选都由互斥的真实 rooted D 值组成，筛选只使用可采纳下界。
void BuildTransposedTerminals(Problem& p, int low_last, int high_last, std::vector<std::vector<int>>& terminal_vertex, std::vector<std::vector<double>>& terminal_value) {
    ...
    for (size_t word = 0; word < word_count; ++word) {
        ...
        for (int vertex = first_vertex; vertex < end_vertex; ++vertex) {
            ...
            auto Potential = [&](auto&& self, int mask) -> double {
                if (subset_stamp[mask] == potential_epoch) return subset_potential[mask];
            ...
            for (size_t left = 0; left + 1 < values.size(); ++left) {
                while (right_limit > left + 1 && values[left].reduced + values[right_limit - 1].reduced > budget)
                    --right_limit;
                if (right_limit <= left + 1) break;
                pair_work += static_cast<long long>(right_limit - left - 1);
                if (pair_work > submask_work) break;
            }

            if (pair_work <= submask_work) {
                for (size_t left = 0; left < values.size(); ++left) {
                    if (left + 1 == values.size() || values[left].reduced + values[left + 1].reduced > budget) break;
                    for (size_t right = left + 1; right < values.size(); ++right) {
                        const double pair_reduced = values[left].reduced + values[right].reduced;
                        if (pair_reduced > budget) break;
                        if (values[left].mask & values[right].mask) continue;
                        const int pair_union = values[left].mask | values[right].mask;
                        const int cover = p.popcount[pair_union];
                        if (cover < minimum_terminal_cover || cover > maximum_terminal_cover) continue;
                        if (direct_auxiliary_layer && cover == minimum_terminal_cover) continue;
                ...
                for (const auto& left : values) {
                    ...
                    for (int right = complement; right; right = (right - 1) & complement) {
                        if (right <= left.mask || value_stamp[right] != value_epoch) continue;
...
// 从补集终端递减计算 H，并与低层 A 及必要的互补半层完成答案。
void SolveHighAdjoint(Problem& p, const std::vector<Row>& anchored, int low_last, int high_last) {
    ...
    auto EvaluateBoundary = [&](int successor) {
        // 按数值升序只生成 successor 的子掩码，直接枚举可能的低层边界。
        for (int mask = 0;; mask = (mask - successor) & successor) {
            ...
            if (mask == successor) break;
        }
    };

    for (int size = high_last; size > low_last; --size) {
        for (int mask = 1; mask < p.subset_count; ++mask) {
            if (p.popcount[mask] != size) continue;
            touched.clear();
            settled.clear();
            ++stamp;
            ...
            auto Prefix = [&](int vertex) {
                if (prefix_stamp[vertex] == stamp) return prefix_cache[vertex];
            ...
            auto Set = [&](int vertex, double value) {
                if (value >= distance[vertex] || !(value + Prefix(vertex) < p.best)) return;
            ...
            for (int block = outside; block; block = (block - 1) & outside) {
                ...
                if (p.popcount[successor] > high_last) continue;
                // 合法 successor 位于已完成的更高 H 层，生命周期检查恒真。
                // lambda：补集侧可能已含规范 branch；新增 ordinary 块读取全部精确值。
                ForEachBackwardValueSum(p, block, backward[successor], [&](int vertex, double value) {
                    Set(vertex, value);
                });
            }
            ...
            while (!queue.empty()) {
                ...
                for (const auto& edge : p.graph.adj[node.vertex]) {
                    ...
                    if (next >= distance[edge.to] || !(next + Prefix(edge.to) < p.best)) continue;
