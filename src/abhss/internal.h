...
template <class Use> void ForEachValue(const Row& row, Use&& use)
{
...
template <class Use> void ForEachBranch(const Row& row, Use&& use)
{
    for (size_t i = 0; i < row.vertex.size(); ++i)
        if (row.IsBranch(i))
            use(row.vertex[i], row.value[i]);
 ...
 *
 * Base 只保留严格小于安全 cutoff 的精确距离，并按实际字节数在 dense
 * 与“有序值+membership/rank 位图”间选择；Enhanced 需要完整势，
...
struct GroupRow
{
    ...
    template <class Use> void ForEachExact(int n, Use&& use) const
    {
        if (!bounded)
        {
            for (int v = 1; v <= n; ++v)
...
/**
 * @brief 距离—候选根职责的统一返回值。
 *
 * `group_distance` 提供相同的值/精确性接口，`root` 供路径并集和锚组选择
 * 使用，`upper` 必须由原图真实边支持。Base 和 Enhanced 只改变函数内部的
 * 距离存储方式，不改变调用点、返回结构或后续控制流。
 */
...
struct Problem
{
     ...
     * vertex 出现在不同状态族时是不同项。两种模式共同提前构造的 A1 row
 ...
 *
 * 两种模式可采用不同的距离表示，但返回结构及后续消费者完全相同。
 */
...
/** @brief 执行共同预处理，并按模式构造对应 witness；返回 true 表示上下界已闭合。 */
...
/** @brief 计算统一 future 下界；Enhanced 额外并入有向割对偶势。 */
...
template <class Use> void ForEachGroupValue(const Problem& p, int mask, Use&& use)
{
...
template <class Use> void ForEachOrdinaryValue(const Problem& p, int mask, Use&& use)
{
...
template <class Use> void ForEachOrdinaryBranch(const Problem& p, int mask, Use&& use)
{
    if (p.popcount[mask] == 1)
        ForEachGroupValue(p, mask, std::forward<Use>(use));
    else
