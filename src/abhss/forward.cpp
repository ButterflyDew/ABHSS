...
namespace abhss {
namespace {
// 枚举 A(anchor_side,v)+D(ordinary_side,v) 的合法同根种子。
// A(0) 和 singleton D 分别直接读取组距离；有界距离必须额外检查 exactmembership。多组 ordinary 只使用规范 branch，避免把可继续同根拆分的中间值重复注入锚定递推。
template <class Use>
void ForEachAnchoredSum(const Problem& p, const std::vector<Row>& anchored, int anchor_side, int ordinary_side, Use&& use) {
    const bool exact_membership_required = !p.enhanced;
    if (!anchor_side) {
...
// 用当前 A(mask,·) 与至多两张 ordinary row 结算完整 GST 上界。
// 先用各 row 最小值做廉价整体拒绝，再从实际候选最少的一侧驱动同根检查。`roots` 与 `anchored_distance` 只在本 row 生命周期内有效；函数不保存引用。
void CompleteAnchoredRow(Problem& p, int mask, const std::vector<int>& roots, const std::vector<double>& anchored_distance, double anchored_minimum) {
    ...
    for (int left = remaining;; left = (left - 1) & remaining) {
        ...
        if (left <= right && p.popcount[left] <= p.half && p.popcount[right] <= p.half && (!left || OrdinaryAvailable(p, left)) && (!right || OrdinaryAvailable(p, right))) {
            ...
            if (anchored_minimum + left_minimum + right_minimum < p.best) {
                ...
                for (int side : {left, right}) {
                    if (!side) continue;
                    const size_t size = p.popcount[side] == 1 ? p.group_distance[
                                                  p.bit_to_group[FirstBit(side)]].ExactSize(p.graph.n) : p.ordinary[side].vertex.size();
                    if (size < driver_size) {
                        driver = side;
                        driver_size = size;
                    }
                ...
                auto Visit = [&](int vertex, bool require_root_membership) {
                    if (require_root_membership && !std::binary_search(roots.begin(), roots.end(), vertex)) return;
                    const double a = left ? OrdinaryValue(p, left, vertex) : 0.0;
                    const double b = right ? OrdinaryValue(p, right, vertex) : 0.0;
                    if (a >= fp::kInf || b >= fp::kInf) return;
...
// 接收已生成的 A1，继续前向 A 并完成答案；只保存后续阶段仍需读取的行。
std::vector<Row> BuildForwardAnchoredRows(Problem& p, int last_size, bool retain_last_layer, std::vector<Row> anchored) {
    if (anchored.empty())
    ...
    // 正层非空时入口至少生成 A1；last_size=0 恰好表示只需隐式 A(0)。
    if (last_size == 0) {
        std::vector<int> roots;
        // lambda：把隐式 A(0) 的全部精确锚组距离装入一次性完成工作区。
        p.group_distance[p.anchor_group].ForEachExact(p.graph.n, [&](int vertex, double value) {
        ...
        for (int mask = 1; mask < p.subset_count; ++mask) {
            ...
            auto CanImprove = [&](int vertex, double value) {
                if (bound_stamp[vertex] != stamp) {
                    ...
                    if (!(value + farthest < p.best)) return false;
            ...
            if (reuses_singleton_row) {
                ...
                ForEachValue(precomputed, [&](int vertex, double value) {
                    if (!CanImprove(vertex, value)) return;
                    distance[vertex] = value;
            ...
            settled.erase(std::remove_if(settled.begin(), settled.end(), [&](int vertex) {
                return !(distance[vertex] + bound_cache[vertex] < p.best);
            }), settled.end());
            if (size < last_size || retain_last_layer) {
