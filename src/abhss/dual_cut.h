...
namespace abhss::dual_cut {
// `DirectedCut` 增强使用的有向割对偶势与 primal 见证容器。
// 按组构造可行对偶势，并从 residual 零弧恢复真实 primal 上界；Base 不构造这些数据。
class DualCutPotential {
public:
    // 构造全部组势、保留 residual，并恢复一棵 primal 可行树。
    // 各组按根距离递减分配有向边容量；每轮只从此前真正改写过的弧启动residual 最短路修复，并只在截断势 cone 支撑上扣减容量。函数保持冷非内联边界；最后在固定容差内的数值零 residual 弧上恢复原图边bitmap，候选路径始终按原边权计价。调用者可在生成 witness 后用 `ReleaseResidual` 回收 2m 临时数组。
    ABHSS_DUAL_NOINLINE void Build(const Graph& graph, const Query& query, const std::vector<std::vector<double>>& group_distance, int root) {
    ...
    // 返回除 excluded_mask 外全部组势之和。
    // residual 购买并转置后，用缓存的全组势减去已覆盖组势；按非负浮点求和的标准 gamma 误差界向下修正，保证返回值不超过直接剩余组和。未转置时退化为原来的剩余 mask 直接求和。
    bool CanImproveAllExcept(int vertex, int excluded_mask, double value, double incumbent, double& lower, bool& exact) const {
        ...
        if (!vertex_potential_.empty()) {
            ...
            if (!(value + lower < incumbent)) return false;
            if (value + upper < incumbent) return true;
    ...
    // 读取一个原始查询组在指定顶点的单组对偶势。
    double GroupAt(int vertex, int group) const {
        if (!vertex_potential_.empty()) return vertex_potential_[static_cast<size_t>(vertex) * potential_group_count_ + group];
        return potential_[group][vertex];
    }

    // 返回数值零 residual 弧上恢复的 primal 可行树真实边权。
    double PrimalUpper() const {
        return primal_upper_;
    }

    // 返回 residual 全势闭包不可避免的图线性 primitive-work 底价。
    // 2m+g(2m+n) 是只由图规模和组数给出的静态扫描底价；另一项逐段计入residual 重放、势补全和 primal 恢复必需的 bitmap、changed-arc、原边与顶点扫描。购买价取两者最大值，不在初始 directed-cut 热循环维护一份以后不再消费的重复工作计数。
    long long ResidualClosureBuyWork(const Graph& graph) const {
        if (potential_.empty() || residual_closure_complete_) return 0;
    ...
    // 从已保存的截断势重建 residual，完成全势闭包并恢复新的 primal 树。
    // 初始预处理已经释放 O(m) residual。本函数只在 ordinary 的真实搜索工作支付上述 buy 后调用一次；重建严格重放各组势对两条有向容量的扣减，随后每组用剩余容量上的精确距离增加一个非负可行势。最终 primal 仍按原图边恢复和计价。调用者读取上界后应立即 ReleaseResidual。
    ABHSS_DUAL_NOINLINE void CompleteResidualClosure(const Graph& graph, const Query& query, const std::vector<std::vector<double>>& group_distance, int root) {
        if (residual_closure_complete_) return;
...
private:
    ...
    // closure 后把完整证书转为按顶点连续布局，并释放按组布局。
    ...
    // 按根距离递减、组号递增并列规则返回统一的 directed-cut 组顺序。
    static std::vector<int> BuildOrder(const std::vector<std::vector<double>>& group_distance, int root) {
        std::vector<int> order(group_distance.size());
        std::iota(order.begin(), order.end(), 0);
        // lambda：锁定根距离递减、组号递增的确定性并列顺序。
        std::sort(order.begin(), order.end(), [&](int left, int right) {
            if (group_distance[left][root] != group_distance[right][root]) return group_distance[left][root] > group_distance[right][root];
            return left < right;
        });
        return order;
    }

    // 重建原始容量并严格重放初始截断势，供两份闭包从同一点独立出发。
    ...
    // 用 changed-arc 修复构造每个组的可行有向割势。
    // 固定按根到组的原始距离递减处理各组。原始组距离对未改写弧满足三角不等式，因此第 t 轮只检查此前势函数改变过的弧，再从违反处正常传播；输出写入 `potential_` 与 `residual_`，不会改动输入距离表。
            ...
            if (cone_degree < graph.edges.size()) {
                ...
                for (int vertex : cone) {
                    for (const AdjEdge& adjacent : graph.adj[vertex]) {
                        ...
                        if (in_cone[adjacent.to] && edge.u != vertex) continue;
    ...
    // 用第一阶段未支付的 residual 容量完成一次全图距离势闭包。
    // 每个增量势是当前 residual 上到该组的精确最短距离；从同向 residual弧扣除其正梯度后，新增势只使用尚未分配的容量。故各组增量可安全累加。固定组顺序只执行一遍：完成一组后每个可达顶点已有到该组的零 residual路径，不需要轮数、收敛阈值或数据相关停止条件。
    void CompleteResidualPotentials(const Graph& graph, const std::vector<std::vector<double>>& group_distance, const std::vector<int>& order, std::vector<std::uint64_t>& changed_arc_words) {
        using HeapItem = std::pair<double, int>;
        ...
        for (int group : order) {
            ...
            for (const UndirectedEdge& edge : graph.edges) {
                if (capped[edge.u] == cap && capped[edge.v] == cap) continue;
                SubtractPositiveGradient(edge, capped, residual_, MarkChangedArc);
            }
        }
    }

    // 在固定容差内的数值零 residual 有向弧上逐次连接未覆盖组。返回：恢复出的原图边并集真实权重；无法覆盖全部组时返回正无穷。
    // Dijkstra 按原边权计价，选中路径写入边位图；residual 容差只影响上界候选，不用于下界剪枝。
    static double RecoverPrimal(const Graph& graph, const Query& query, int root, const std::vector<double>& residual, std::vector<std::uint64_t>& primal_edge_words) {
        using HeapItem = std::pair<double, int>;
        ...
        while (covered != full_mask) {
            ...
            while (!heap.empty()) {
                ...
                for (const AdjEdge& edge : graph.adj[vertex]) {
                    ...
                    if (residual[arc] > tolerance) continue;
