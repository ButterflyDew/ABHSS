...
// 返回 A1 cone 与非负 fallback 共同使用的剩余代价下界。
// farthest 与 endpoint-floor 都可采纳且沿边至多下降边权；取最大后仍保持该性质，因此既能安全决定 Dijkstra cone，也能用于 cone 外的 U-B fallback。
...
// 任一安全证书严格加强后删除已缓存 row 中不再可能改善 incumbent 的状态。
void RefilterOrdinaryAfterCertificateUpgrade(Problem& p) {
    for (int mask = 1; mask < p.subset_count; ++mask) {
        Row& row = p.ordinary[mask];
        if (!row.ready || row.vertex.empty()) continue;
...
// 只登记 witness 的结构成本；首次树 DP 留待累计搜索工作达到购买点。
WitnessUpperScheduler::WitnessUpperScheduler(Problem& problem) : problem_(problem) {
    buy_ = problem_.certificate_support_edges.empty() ? EstimateWitnessTreeDpWork(problem_.witness_tree.vertex.size(), problem_.nonanchor_count) : EstimateCertificateSupportDpWork(problem_.certificate_support_vertex_count, problem_.nonanchor_count);
    enabled_ = buy_ > 0;
...
// 累计 A1/D 的实际工作；新输入且租金足够时求一次可行上界。
bool WitnessUpperScheduler::Account(long long row_work, bool ordinary_changed) {
    ...
    if (rent_ < buy_ || evaluated_revision_ == ordinary_revision_) return false;

    const long long paid_rent = rent_;
    const double old_best = problem_.best;
    Evaluate();
    if (problem_.best < old_best && ordinary_revision_ > 0) {
        RefilterOrdinaryAfterCertificateUpgrade(problem_);
        if (support_dp_)
            support_dp_->Reset();
    }
    purchased_rent_ = paid_rent >= std::numeric_limits<long long>::max() - purchased_rent_ ? std::numeric_limits<long long>::max() : purchased_rent_ + paid_rent;
    rent_ = 0;
    evaluated_revision_ = ordinary_revision_;
    return problem_.best < old_best;
}

// 返回已经累计的搜索工作，供 Enhanced 的一次性证书刷新接续计费。
long long WitnessUpperScheduler::TotalWork() const {
    if (!problem_.enhanced) return 0;
    return rent_ >= std::numeric_limits<long long>::max() - purchased_rent_ ? std::numeric_limits<long long>::max() : purchased_rent_ + rent_;
}

// 计算本轮距离下一次可购买还剩多少工作，供 A1 在堆循环中结算。
long long WitnessUpperScheduler::RemainingRentUntilBuy() const {
    if (!enabled_ || evaluated_revision_ == ordinary_revision_) return std::numeric_limits<long long>::max();
    return rent_ >= buy_ ? 0 : buy_ - rent_;
}

// 用同一入口求当前树或已更新支撑图上的可行上界。
...
// 支撑图变更后更换 evaluator，重新登记其结构成本并从零累计。
void WitnessUpperScheduler::RefreshCertificate() {
    buy_ = problem_.certificate_support_edges.empty() ? EstimateWitnessTreeDpWork(problem_.witness_tree.vertex.size(), problem_.nonanchor_count) : EstimateCertificateSupportDpWork(problem_.certificate_support_vertex_count, problem_.nonanchor_count);
    rent_ = 0;
    evaluated_revision_ = ordinary_revision_;
    enabled_ = buy_ > 0;
    support_dp_ = problem_.certificate_support_edges.empty() ? nullptr : std::make_unique<CertificateSupportDpCache>(problem_);
}

// Enhanced 登记一次 residual 全势闭包的静态成本；Base 不启用。
ResidualClosureScheduler::ResidualClosureScheduler(Problem& problem) : problem_(problem) {
    if (problem_.enhanced)
        buy_ = problem_.dual.ResidualClosureBuyWork(problem_.graph);
    enabled_ = buy_ > 0;
}

// 累计搜索工作和已发布状态数；两者达到结构购买条件后刷新一次证书。
bool ResidualClosureScheduler::Account(long long row_work, long long new_payload) {
    if (!enabled_ || purchased_) return false;
    ...
    if (row_work >= std::numeric_limits<long long>::max() - rent_)
        ...
        rent_ += row_work;
    if (rent_ < buy_) return false;
    if (payload_ < problem_.graph.n) return false;
    return Buy();
}

// 补全 residual 势，尝试更紧的真实上界，再按加强后的证书重滤普通行。
bool ResidualClosureScheduler::Buy() {
    ...
    problem_.dual.CompleteResidualClosure(problem_.graph, problem_.query, dense, problem_.root);
    for (int group = 0; group < problem_.g; ++group)
        dense[group].swap(problem_.group_distance[group].value);
...
// 一次二分同时返回 A1 值和下标；cone 外用高位标记其公式来源。
...
// 由固定 cone 截止值减去连续补全部分，得到 cone 外的安全下界。
...
// 复用已知下标读取原 double，或重新计算 cone 外下界。
double AnchoredSingletonFuture::LocatedValue(const Problem& p, int bit, int vertex, std::uint32_t locator) const {
    if (!(locator & kA1FallbackLocator)) return row[bit].value[locator];
    return FallbackValue(p, bit, vertex);
}

// 按已发布 A1 行的长度，计算 top-two 与完整排名两级缓存的购买点。
...
// 顺序扫描全部 A1 行，物化每个顶点的前两名和每个 mask 的尾部查询成本。
...
// 为每个顶点排序余下的 singleton 编号；只存 byte 次序，不复制距离值。
...
double AnchoredSingletonFuture::Future(const Problem& p, int remaining, int vertex) {
    ...
    if (a != 255 && (remaining & (1 << a))) return LocatedValue(p, 1 << a, vertex, static_cast<std::uint32_t>(cached_locator_pair[vertex]));
    const int b = second[vertex];
    if (b != 255 && (remaining & (1 << b))) return LocatedValue(p, 1 << b, vertex, static_cast<std::uint32_t>(cached_locator_pair[vertex] >> 32));
    ...
    if (!ranked_tail.empty()) {
        ...
        for (int rank = 0; rank < tail_size; ++rank) {
            ...
            if (remaining & (1 << bit_index)) return Value(p, 1 << bit_index, vertex);
        }
        return 0.0;
    }
...
// 共同构造可提前复用的 A1；树 DP 收紧上界时丢弃当前 pass，以新 cutoff 重启。
        ...
        for (int mask = 1; mask < p.subset_count; mask <<= 1) {
            ...
            auto Continuation = [&](int vertex) {
                if (continuation_stamp[vertex] != stamp) {
                    ...
                    continuation_cache[vertex] = AnchoredSingletonContinuation(p, vertex, continuation);
                }
            ...
            p.group_distance[group].ForEachExact(p.graph.n, [&](int vertex, double value) {
                ...
                if (anchor_distance >= fp::kInf) return;
                const double candidate = value + anchor_distance;
                if (!(candidate < singleton_future.cutoff) || candidate >= distance[vertex]) return;
            ...
            while (!queue.empty()) {
                ...
                if (node.distance != distance[node.vertex] || !(node.distance < singleton_future.cutoff) || !(node.key < p.best)) continue;
                settled.push_back(node.vertex);
                for (const auto& edge : p.graph.adj[node.vertex]) {
                    ++row_work;
                    const double next = node.distance + edge.w;
                    if (!(next < singleton_future.cutoff) || next >= distance[edge.to]) continue;
                    const double key = next + Continuation(edge.to);
                    if (!(key < p.best)) continue;
...
namespace {
// 枚举三个 ordinary 分块与永久锚组在共同根相遇的可行解。
// 从实际可枚举值最少的一侧驱动，缺失状态立即拒绝。该完成式在三分块大小达到平衡点时收紧上界，但不替代后续精确状态枚举。
template <class Use>
...
// 按子集大小合并、图闭包和发布 D 行；模板只区分完整与分阶段下界缓存。
void BuildOrdinaryRowsImpl(Problem& p, AnchoredSingletonFuture* singleton_future, WitnessUpperScheduler& witness_scheduler, ResidualClosureScheduler& closure_scheduler, int last_layer) {
    std::vector<double> distance(p.graph.n + 1, fp::kInf);
    std::vector<double> split(p.graph.n + 1, fp::kInf);
    std::vector<double> bound_cache(p.graph.n + 1);
    // Base 继续使用单一 stamp；Enhanced将 row epoch、证书阶段和两个拒绝前沿位
    // 打包在一个 32-bit 字中。
    ...
    for (int size = 1; size <= last_layer; ++size) {
        for (int mask = 1; mask < p.subset_count; ++mask) {
            if (p.popcount[mask] != size || size == 1) continue;
            long long row_work = 0;
            touched.clear();
            settled.clear();
            rejected.clear();
            ...
            // lambda：Base 一次缓存完整公共 future；Enhanced逐级缓存
            ...
            auto CanImprove = [&](int vertex, double value) {
                if constexpr (!kStagedCertificateCache) {
                    if (bound_stamp[vertex] != stamp) {
                        ...
                        if (!(value + lower < p.best)) return false;
                        if (singleton_future) {
                ...
                if ((state & kBoundStageMask) == 0) {
                    ...
                    if (!dual_can_improve) {
                        ...
                        // 后续更小标签才能复用同一 dual，保持每次候选的原始接受谓词。
                        if (exact_dual) {
            ...
            auto Set = [&](int vertex, double value) {
                if (value >= distance[vertex] || !CanImprove(vertex, value)) return;
                if (distance[vertex] >= fp::kInf)
                    touched.push_back(vertex);
                distance[vertex] = value;
            };
            ...
            while (!queue.empty()) {
                ...
                queue.pop();
                ++row_work;
                if (node.distance != distance[node.vertex] || !(node.key < p.best)) continue;
                settled.push_back(node.vertex);
                for (const auto& edge : p.graph.adj[node.vertex]) {
                    ++row_work;
                    const double next = node.distance + edge.w;
                    if (next >= distance[edge.to] || !CanImprove(edge.to, next)) continue;
                    if (distance[edge.to] >= fp::kInf)
                        touched.push_back(edge.to);
                    distance[edge.to] = next;
            ...
            if (size == p.half) {
                ...
                if (p.popcount[complement] < size || (p.popcount[complement] == size && complement < mask)) {
            ...
            if (size == three_block_limit) {
                ...
                for (int second = remaining;; second = (second - 1) & remaining) {
                    ...
                    if (second <= third && p.popcount[second] <= size && p.popcount[third] <= size && (!second || OrdinaryAvailable(p, second)) && (!third || OrdinaryAvailable(p, third))) {
...
// 一次选择下界缓存实现，避免每次状态访问都判断模式。
void BuildOrdinaryRows(Problem& p, AnchoredSingletonFuture* singleton_future, WitnessUpperScheduler& witness_scheduler, ResidualClosureScheduler& closure_scheduler, int last_layer) {
    if (p.enhanced)
